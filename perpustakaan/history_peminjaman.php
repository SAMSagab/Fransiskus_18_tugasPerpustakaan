...
<table class="table table-hover table-striped">
    ...
    <tbody>
        ...
        while($data_histori=mysqli_fetch_array($qry_histori)){
            ...
            $buku_dipinjam="<ol>";
            $qry_buku=mysqli_query($conn,"select * from  detail_peminjaman_buku join buku on buku.id_buku=detail_peminjaman_buku.id_buku where id_peminjaman_buku = '".$data_histori['id_peminjaman_buku']."'");
            ...
            $qry_cek_kembali=mysqli_query($conn,"select * from pengembalian_buku where id_peminjaman_buku = '".$data_histori['id_peminjaman_buku']."'");
            if(mysqli_num_rows($qry_cek_kembali)>0){
                ...
                $button_kembali="<a href='kembali.php?id=".$data_histori['id_peminjaman_buku']."' class='btn btn-warning' onclick='return confirm(\"Yakin kah?\")'>Kembalikan</a>";
            }
        ?>
            <tr>
                <td><?=$no?></td>
